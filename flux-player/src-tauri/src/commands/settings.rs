...
#[tauri::command]
pub async fn optimize_database<R: Runtime>(app: AppHandle<R>) -> AppResult<()> {
    let db_path = crate::database::connection::get_db_path(&app)?;
    let conn = rusqlite::Connection::open(&db_path)
...
pub async fn open_uninstaller<R: Runtime>(app: AppHandle<R>) -> AppResult<()> {
    #[cfg(target_os = "windows")]
    {
        // 1. TRY PRODUCTION MODE (Registry Lookup)
        // Check both HKLM and HKCU for the uninstall string
        ...
        if let Ok(cmd) = uninstaller_cmd {
            let cmd = cmd.trim();
            if !cmd.is_empty() {
                let (exe_str, rest) = if cmd.starts_with('"') {
                    if let Some(end_idx) = cmd[1..].find('"') {
                        (&cmd[1..=end_idx], &cmd[end_idx + 2..])
                    } else {
                        (&cmd[1..], "")
                    }
                } else if let Some(space_idx) = cmd.find(" /") {
                    (&cmd[..space_idx], &cmd[space_idx + 1..])
                } else if let Some(space_idx) = cmd.find(" -") {
                    (&cmd[..space_idx], &cmd[space_idx + 1..])
                } else if cmd.to_lowercase().ends_with(".exe") {
                    (cmd, "")
                } else {
                    (cmd, "")
                };
                ...
                if path.is_absolute() && path.exists() {
                    if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                        ...
                        if valid_exts.contains(&ext.to_lowercase().as_str()) {
                            println!(
                                "[Flux Uninstaller] Executing safe registry uninstaller: {}",
                                exe_str
                            );
