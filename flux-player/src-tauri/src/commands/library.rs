...
pub async fn cache_tmdb_image<R: Runtime>(
    ...
    // Security: Prevent path traversal by strict allowlisting of image_type
    ...
    // Security: Sanitize file extension to prevent injection and limit length
    let raw_ext = url.split('.').next_back().unwrap_or("jpg");
    let base_ext = raw_ext.split('?').next().unwrap_or("jpg");
    let mut clean_ext: String = base_ext
        .chars()
        .filter(|c| c.is_ascii_alphanumeric())
        .take(10)
        .collect();
    if clean_ext.is_empty() {
        clean_ext = "jpg".to_string();
    }
