...
pub async fn cache_tmdb_image<R: Runtime>(
    ...
    // SECURITY FIX: Prevent path traversal by allowing only specific image types
    let allowed_types = ["posters", "backdrops", "album-art"];
    ...
    // SECURITY FIX: Sanitize file extension to prevent injection or weird file creations
    let mut file_extension = url.split('.').next_back().unwrap_or("jpg");
    if file_extension.len() > 5 || !file_extension.chars().all(char::is_alphanumeric) {
        file_extension = "jpg";
    }
