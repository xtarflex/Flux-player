...
pub async fn cache_tmdb_image<R: Runtime>(
    ...
    // 🛡️ Sentinel: Validate image_type to prevent path traversal
    if image_type != "posters" && image_type != "backdrops" && image_type != "album-art" {
        return Err(crate::utils::error::AppError::InvalidInput(
    ...
    // 🛡️ Sentinel: Sanitize file extension to prevent execution/injection risks
    let mut file_extension = url.split('.').next_back().unwrap_or("jpg");
    if !file_extension.chars().all(char::is_alphanumeric)
        || file_extension.len() > 4
        || file_extension.len() < 2
    {
        file_extension = "jpg";
    }
