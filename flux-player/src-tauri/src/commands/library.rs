...
pub async fn cache_tmdb_image<R: Runtime>(
    ...
    // Security enhancement: Validate image_type against allowlist to prevent path traversal
    match image_type.as_str() {
        "posters" | "backdrops" | "album-art" => {}
        _ => {
            return Err(crate::utils::error::AppError::InvalidInput(
    ...
    // Security enhancement: Sanitize file_extension to prevent malicious extensions
