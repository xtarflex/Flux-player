...
pub fn get_system_media_folders() -> Vec<(String, String)> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(p) = video_path {
            folders.push((p, "video".to_string()));
        }
        if let Some(p) = music_path {
            folders.push((p, "audio".to_string()));
        }
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if std::path::Path::new(&video).exists() {
            folders.push((video, "video".to_string()));
        }
        if std::path::Path::new(&music).exists() {
            folders.push((music, "audio".to_string()));
        }
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let video = std::env::var("XDG_VIDEOS_DIR").unwrap_or_else(|_| format!("{}/Videos", home));
        let music = std::env::var("XDG_MUSIC_DIR").unwrap_or_else(|_| format!("{}/Music", home));
        if std::path::Path::new(&video).exists() {
            folders.push((video, "video".to_string()));
        }
        if std::path::Path::new(&music).exists() {
