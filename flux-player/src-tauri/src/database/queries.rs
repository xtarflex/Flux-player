...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        let tx = conn.transaction()?;
        {
            ...
            // Prepared statements are extracted out of the loop.
            // This prevents the query planner from re-parsing and re-optimizing the SQL statement on every iteration.
