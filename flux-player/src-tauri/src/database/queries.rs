...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // Bolt: Optimize bulk delete by preparing statement outside the loop.
            // Avoids redundant parsing/compilation overhead for each deleted item.
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // Bolt: Optimize bulk insert by preparing statement outside the loop.
        // Reduces parsing overhead which is significant when scanning large directories.
