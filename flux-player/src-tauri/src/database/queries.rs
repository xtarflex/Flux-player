...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // Bolt: Optimize bulk database operation by preparing the statement outside the loop.
            // This prevents parsing and compiling the SQL on every iteration.
...
pub fn save_media_items<R: Runtime>(
...

    {
        ...
        // Pre-compiling the INSERT statement outside the loop avoids redundant parsing and query planning.
