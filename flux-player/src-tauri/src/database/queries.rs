...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // Performance: Prepare the SQL statement once outside the loop
            // to avoid re-parsing overhead for bulk deletions.
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // Performance: Prepare the complex INSERT ... ON CONFLICT statement once
        // outside the loop to significantly reduce parsing overhead during bulk saves.
