...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // ⚡ Bolt Optimization: Prepare the DELETE statement outside the loop to avoid redundant query parsing.
            // Wrapping it in a block ensures the statement is dropped, releasing the mutable borrow on the transaction
            // before `tx.commit()` is called. Expected impact: significantly faster batch deletions.
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // ⚡ Bolt Optimization: Prepare the INSERT ... ON CONFLICT statement outside the loop to avoid re-compiling
        // the SQL string on every iteration. Wrapping it in a block ensures the mutable borrow on `tx` is released
        // before the commit. Expected impact: Much lower database overhead during bulk media saves.
...
mod tests {
    ...
    fn test_clean_stale_media() {
        ...
        if !stale_paths.is_empty() {
            ...
            {
                // Mirror the performance optimization from `clean_stale_media`
