...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // ⚡ Bolt: Prepare the DELETE statement outside the loop to avoid redundant compilation overhead
            ...
            // Explicitly drop statement to release mutable borrow on transaction
        }
        tx.commit()?;
    }
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // ⚡ Bolt: Prepare the UPSERT statement outside the loop to avoid redundant compilation overhead
        // during bulk operations. This significantly improves performance when saving many items.
