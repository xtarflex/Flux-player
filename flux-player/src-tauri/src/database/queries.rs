...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // Optimize bulk deletes by preparing the SQL statement outside the loop to avoid redundant parsing overhead on every iteration
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // Optimize bulk inserts by preparing the SQL statement outside the loop to avoid redundant parsing overhead on every iteration
