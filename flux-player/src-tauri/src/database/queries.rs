...
pub fn clean_stale_media<R: Runtime>(app: &AppHandle<R>, dir_path: &str) -> AppResult<usize> {
    ...
    if !stale_paths.is_empty() {
        ...
        {
            // Bolt: Extracted `prepare` outside the loop to avoid redundant query parsing.
            // Expect significant performance improvement on bulk delete operations.
            // Statement is wrapped in a block scope to release mutable borrow on `tx` before `tx.commit()`.
...
pub fn save_media_items<R: Runtime>(
    ...
    {
        // Bolt: Extracted `prepare` outside the loop to avoid redundant query parsing.
        // Moving this parsing out-of-loop significantly improves performance for bulk media saves (e.g. initial scan).
        // The block scope ensures `stmt` is dropped and `tx` borrow is released before `tx.commit()`.
