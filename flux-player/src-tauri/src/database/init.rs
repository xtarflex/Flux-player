    ...
    fn test_migrations_full_chain() {
        ...
        for migration in &migrations {
            ...
            conn.execute_batch(migration.sql)
                .expect(&format!("Migration {} ({}) failed to apply", migration.version, migration.description));
        ...
        assert!(column_names.contains(&"rating".to_string()), "Migration v6 (rating) missing");
        assert!(column_names.contains(&"genres".to_string()), "Migration v7 (genres) missing");
        assert!(column_names.contains(&"series_tag".to_string()), "Migration v10 (series_tag) missing");
        assert!(column_names.contains(&"is_watched".to_string()), "Migration v10 (is_watched) missing");
        assert!(column_names.contains(&"last_position".to_string()), "Migration v11 (playback) missing");
        assert!(column_names.contains(&"is_favorite".to_string()), "Migration v12 (favorites) missing");

        println!("[Flux DB Test] All {} migrations verified successfully.", migrations.len());
    }
}
